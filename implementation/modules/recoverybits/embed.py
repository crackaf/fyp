...
def embedfrom8x8(b8x8: np.ndarray, recovery_bits: np.ndarray) -> np.ndarray:
...
def embed(imarr: np.ndarray, recovery_bits: np.ndarray) -> np.ndarray:
    ...
    for i in range(b16x16.shape[0]):
        for j in range(b16x16.shape[2]):
            b8x8[i][0][j] = get_blocks(b16x16[i][0][j].copy(), b8x8_shape,
                                       addChannel=False)

    ################################################################################

    embedfrom8x8(b8x8, recovery_bits)
    ...
    for i in range(c16x16.shape[0]):
        for j in range(c16x16.shape[2]):
            c16x16[i][0][j] = combine_blocks(b8x8[i][0][j].copy(), imageshape=(
    ...
    return combine_blocks(cmainblocks.copy(), imarr.shape,
