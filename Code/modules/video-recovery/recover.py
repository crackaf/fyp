...
def _recover(imarr: np.ndarray, recovery_bits: np.ndarray,
             tempred: np.ndarray, lookup: np.ndarray, interpolation: int = cv2.INTER_CUBIC) -> RecoveryResult:
...
def recover(imarr: np.ndarray, lookup: np.ndarray, interpolation: int = cv2.INTER_CUBIC) -> RecoveryResult:
